import { useEffect, useMemo, useState } from "react"
import { ChevronDown, ListTodo, Plus, Trash2 } from "lucide-react"
import { toast } from "react-toastify"
import { Button } from "@/components/ui/button"
import {
  baseInputClass,
  cardBaseClass,
  cardBodyClass,
  cardHeaderBaseClass,
  centerInputClass,
  selectWrapperClass,
  tableCellClass,
  tableHeaderClass,
} from "@/components/project/projectDetailStyles"
import type { EditableTodoItem } from "@/components/project/projectDetailTypes"
import {
  createProjectOptionAPI,
  createProjectTodoAPI,
  deleteProjectTodoAPI,
  updateProjectTodoAPI,
} from "@/services/apis"

type ProjectTodoListProps = {
  projectId: number
  isEditing: boolean
  items: EditableTodoItem[]
  statusOptions: string[]
  onStatusOptionsChange: (statuses: string[]) => void
  onItemsChange: (items: EditableTodoItem[]) => void
  onError: (error: unknown) => void
}

// 待辦項目可快速選擇的狀態。
const TODO_STATUS_OPTIONS = ["進行中", "已完成"]

// 新增待辦表單的預設值。
const emptyTodo = {
  item: "",
  assignee: "",
  status: "尚未開始",
  dueDate: "",
  note: "",
}

const ProjectTodoList = ({
  projectId,
  isEditing,
  items,
  statusOptions,
  onStatusOptionsChange,
  onItemsChange,
  onError,
}: ProjectTodoListProps) => {
  // 管理新增待辦表單、狀態選單與目前正在儲存的列。
  const [newTodo, setNewTodo] = useState(emptyTodo)
  const [activeStatusPicker, setActiveStatusPicker] = useState<string | null>(null)
  const [savingIds, setSavingIds] = useState<Set<number>>(new Set())

  // 狀態選項由後端選項與目前輸入值組成，避免和專案主表狀態互相影響。
  const todoStatusOptions = useMemo(() => {
    const values = [
      ...statusOptions,
      newTodo.status,
    ]
    return Array.from(new Set(values.map((status) => status.trim()).filter(Boolean)))
  }, [newTodo.status, statusOptions])

  // 新狀態會正式寫入後端選項表，之後其他專案也能從 API 撈到。
  const ensureStatusOption = async (status: string) => {
    const value = status.trim()
    if (!value || statusOptions.includes(value)) return

    const response = await createProjectOptionAPI("todo_status", value)
    if (response.status === -1) {
      onError(response.error)
      return
    }
    onStatusOptionsChange([...statusOptions, value])
  }

  useEffect(() => {
    // 離開編輯模式時清空新增待辦與狀態選單，避免殘留未送出的操作。
    if (!isEditing) {
      setNewTodo(emptyTodo)
      setActiveStatusPicker(null)
    }
  }, [isEditing])

  // 延遲關閉狀態選單，讓滑鼠點選選項事件可以先完成。
  const closeStatusPickerSoon = () => {
    window.setTimeout(() => setActiveStatusPicker(null), 120)
  }

  // 記錄單列是否正在儲存，用來暫時停用該列輸入。
  const setSaving = (id: number, saving: boolean) => {
    setSavingIds((prev) => {
      const next = new Set(prev)
      if (saving) {
        next.add(id)
      } else {
        next.delete(id)
      }
      return next
    })
  }

  // 新增待辦項目並把後端回傳資料同步到父層清單。
  const handleAdd = async () => {
    const item = newTodo.item.trim()
    if (!item) {
      toast.error("待辦事項不可為空", { position: "top-center" })
      return
    }

    const response = await createProjectTodoAPI(projectId, {
      item,
      assignee: newTodo.assignee.trim(),
      status: newTodo.status.trim() || "尚未開始",
      dueDate: newTodo.dueDate,
      note: newTodo.note.trim(),
    })

    if (response.status === -1) {
      onError(response.error)
      return
    }

    await ensureStatusOption(response.data.status)
    onItemsChange([...items, response.data])
    setNewTodo(emptyTodo)
    setActiveStatusPicker(null)
  }

  // 先在前端即時更新指定欄位，等 blur 或選單選擇時再送出儲存。
  const handleChange = (rowId: number, key: keyof Omit<EditableTodoItem, "id">, value: string) => {
    onItemsChange(items.map((row) => (row.id === rowId ? { ...row, [key]: value } : row)))
  }

  // 欄位離開焦點時儲存該筆待辦。
  const handleBlur = async (row: EditableTodoItem) => {
    await saveRow(row)
  }

  // 驗證並更新單筆待辦資料。
  const saveRow = async (row: EditableTodoItem) => {
    if (!row.item.trim()) {
      toast.error("待辦事項不可為空", { position: "top-center" })
      return
    }

    setSaving(row.id, true)
    const response = await updateProjectTodoAPI(projectId, row.id, {
      item: row.item.trim(),
      assignee: row.assignee.trim(),
      status: row.status.trim() || "尚未開始",
      dueDate: row.dueDate,
      note: row.note.trim(),
    })
    setSaving(row.id, false)

    if (response.status === -1) {
      onError(response.error)
      return
    }

    await ensureStatusOption(response.data.status)
    onItemsChange(items.map((item) => (item.id === row.id ? response.data : item)))
  }

  // 選擇狀態後立即更新前端清單並送出儲存。
  const handleStatusSelect = async (row: EditableTodoItem, status: string) => {
    setActiveStatusPicker(null)
    const nextRow = { ...row, status }
    onItemsChange(items.map((item) => (item.id === row.id ? nextRow : item)))
    await saveRow(nextRow)
  }

  // 刪除指定待辦，成功後同步移除父層清單資料。
  const handleDelete = async (rowId: number) => {
    const response = await deleteProjectTodoAPI(projectId, rowId)
    if (response.status === -1) {
      onError(response.error)
      return
    }
    onItemsChange(items.filter((row) => row.id !== rowId))
  }

  return (
    <section className={`${cardBaseClass} border-t-amber-500`}>
      <div className={`${cardHeaderBaseClass} bg-amber-50`}>
        <div className="flex items-center gap-3">
          <ListTodo className="h-6 w-6 text-amber-600" />
          <h3 className="text-amber-950">須完成項目</h3>
        </div>
      </div>

      <div className={cardBodyClass}>
        {isEditing && (
          <div className="mb-8 rounded-lg border border-slate-300 bg-slate-100 p-5">
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(14rem,2fr)_minmax(10rem,1fr)_minmax(10rem,1fr)_12rem_minmax(10rem,1fr)_auto]">
              <input className={baseInputClass} placeholder="項目" value={newTodo.item} onChange={(e) => setNewTodo({ ...newTodo, item: e.target.value })} />
              <input className={baseInputClass} placeholder="項目負責人" value={newTodo.assignee} onChange={(e) => setNewTodo({ ...newTodo, assignee: e.target.value })} />
              <div className={selectWrapperClass}>
                <input className={`${baseInputClass} pr-8`} placeholder="狀態" value={newTodo.status} onBlur={closeStatusPickerSoon} onChange={(e) => setNewTodo({ ...newTodo, status: e.target.value })} onFocus={() => setActiveStatusPicker("new")} />
                <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-500" />
                {activeStatusPicker === "new" && (
                  <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-md border border-slate-300 bg-white shadow-lg">
                    {todoStatusOptions.map((status) => (
                      <button key={status} type="button" className="block w-full px-3 py-2 text-left text-base font-semibold text-slate-800 transition-colors hover:bg-slate-100" onMouseDown={(e) => e.preventDefault()} onClick={() => { setNewTodo({ ...newTodo, status }); setActiveStatusPicker(null) }}>
                        {status}
                      </button>
                    ))}
                  </div>
                )}
              </div>
              <input type="date" className={baseInputClass} value={newTodo.dueDate} onChange={(e) => setNewTodo({ ...newTodo, dueDate: e.target.value })} />
              <input className={baseInputClass} placeholder="備註" value={newTodo.note} onChange={(e) => setNewTodo({ ...newTodo, note: e.target.value })} />
              <Button type="button" className="h-[46px] whitespace-nowrap bg-slate-800 px-6 text-base font-bold text-white hover:bg-slate-900" onClick={handleAdd}>
                <Plus className="mr-2 h-5 w-5" /> 新增
              </Button>
            </div>
          </div>
        )}

        <div className="overflow-x-auto rounded-lg border border-slate-300">
          <table className="w-full text-base">
            <thead>
              <tr>
                <th className={`${tableHeaderClass} w-[34%]`}>項目</th>
                <th className={`${tableHeaderClass} w-[12%]`}>項目負責人</th>
                <th className={`${tableHeaderClass} w-[12%]`}>狀態</th>
                <th className={`${tableHeaderClass} w-[14%]`}>須完成時間</th>
                <th className={tableHeaderClass}>備註</th>
                {isEditing && <th className={`${tableHeaderClass} w-[8%]`}>操作</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 bg-white">
              {items.length === 0 ? (
                <tr>
                  <td colSpan={isEditing ? 6 : 5} className="py-10 text-center font-medium text-slate-500">尚未新增須完成項目</td>
                </tr>
              ) : (
                items.map((row) => (
                  <tr key={row.id} className="transition-colors hover:bg-slate-50">
                    <td className={tableCellClass}><input className={centerInputClass} value={row.item} disabled={!isEditing || savingIds.has(row.id)} onBlur={() => handleBlur(row)} onChange={(e) => handleChange(row.id, "item", e.target.value)} /></td>
                    <td className={tableCellClass}><input className={centerInputClass} value={row.assignee} disabled={!isEditing || savingIds.has(row.id)} onBlur={() => handleBlur(row)} onChange={(e) => handleChange(row.id, "assignee", e.target.value)} /></td>
                    <td className={tableCellClass}>
                      <div className={selectWrapperClass}>
                        <input className={`${centerInputClass} pr-8 disabled:pr-0`} value={row.status} disabled={!isEditing || savingIds.has(row.id)} onBlur={() => { closeStatusPickerSoon(); void handleBlur(row) }} onChange={(e) => handleChange(row.id, "status", e.target.value)} onFocus={() => setActiveStatusPicker(`row-${row.id}`)} />
                        {isEditing && <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-500" />}
                        {isEditing && activeStatusPicker === `row-${row.id}` && (
                          <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-md border border-slate-300 bg-white shadow-lg">
                            {todoStatusOptions.map((status) => (
                              <button key={status} type="button" className="block w-full px-3 py-2 text-left text-base font-semibold text-slate-800 transition-colors hover:bg-slate-100" onMouseDown={(e) => e.preventDefault()} onClick={() => void handleStatusSelect(row, status)}>
                                {status}
                              </button>
                            ))}
                          </div>
                        )}
                      </div>
                    </td>
                    <td className={tableCellClass}><input type={isEditing ? "date" : "text"} className={centerInputClass} value={row.dueDate} disabled={!isEditing || savingIds.has(row.id)} onBlur={() => handleBlur(row)} onChange={(e) => handleChange(row.id, "dueDate", e.target.value)} /></td>
                    <td className={tableCellClass}><input className={centerInputClass} value={row.note} disabled={!isEditing || savingIds.has(row.id)} onBlur={() => handleBlur(row)} onChange={(e) => handleChange(row.id, "note", e.target.value)} /></td>
                    {isEditing && (
                      <td className={`${tableCellClass} text-center`}>
                        <button type="button" onClick={() => handleDelete(row.id)} className="rounded-md p-2 text-red-600 transition-colors hover:bg-red-50 hover:text-red-700">
                          <Trash2 className="h-5 w-5" />
                        </button>
                      </td>
                    )}
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
    </section>
  )
}

export default ProjectTodoList
