...
// 可以建立自己表格的角色
const CAN_CREATE_ROLES = ["rd_user", "rd_leader", "super", "boss"];

export default function RDWeeklyReport() {
  ...
  const handleDeleteTable = async (tableId: number) => {
    ...
    try {
      const response = await axios.delete(`/api/rd/tables/${tableId}`, getRequestConfig());
      if (response.data && response.data.status === 0) {
        fetchWeeklyData(currentWeek);
      }
    } catch (error) {
